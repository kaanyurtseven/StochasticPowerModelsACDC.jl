...
""
function solve_sopf_acdc_PV(file::String, model_constructor, optimizer; deg::Int=1, p_size=0, solution_processors=[sol_data_model!], kwargs...)
    data = _PM.parse_file(file)
    process_additional_data!(data)
    
    return solve_sopf_acdc_PV(data, model_constructor, optimizer; deg=deg, p_size=p_size, ref_extensions = [_PMACDC.add_ref_dcgrid!], solution_processors=solution_processors, kwargs...)
end

""
function solve_sopf_acdc_PV(data::Dict, model_constructor, optimizer; deg::Int=1, p_size=0, solution_processors=[sol_data_model!], kwargs...)
    ...
    sdata = build_stochastic_acdc_data_PV(data, deg, p_size)
    result = _PM.solve_model(sdata, model_constructor, optimizer, build_sopf_acdc_PV; multinetwork=true, ref_extensions = [_PMACDC.add_ref_dcgrid!], solution_processors=solution_processors, kwargs...)
...
""
function build_sopf_acdc_PV(pm::AbstractPowerModel)
    for (n, network) in _PM.nws(pm) 
        ...
        variable_PV_current(pm, nw=n, bounded=bounded)

        # if n==1
        #     variable_PV_size(pm, nw=n, bounded=true)
        #     variable_PV_size_imaginary(pm, nw=n, bounded=true)
        # end

    end
    
    objective_min_expected_generation_cost(pm)
    #objective_min_expected_generation_cost_PV(pm)
    #objective_max_PV(pm)
    #objective_max_PV_imaginary(pm)
